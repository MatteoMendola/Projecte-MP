...
void Joc::inicialitza(int mode, const string& nomFitxer) //inicialitza l'estat d'una partida a partir d'un fitxer
{
	if (mode == 1) //partida mode normal
	{
		m_tauler.inicialitzar();
		m_figuraActual.inicialitzar();
	}
	else  //partida mode test
	{
		ifstream fitxer;
...
int Joc::baixaFigura(bool& esPot) //retorna un 'int' amb el nombre de files eliminades
{
...
void Joc::baixarFinal()
{
	int fila = 0, col = 0;
	TipusFigura tipus = m_figuraActual.getTipus();
	int nCasMax = m_figuraActual.nombreCaselles(tipus);
	m_figuraActual.calcularPosicioTauler(fila, col);
	int i = N_FILES_TAULER - 1;
	bool trobat = false;
	bool matriuAux[MAX_ALCADA][MAX_AMPLADA];
	m_figuraActual.getMatriuFormaAct(matriuAux);

	while (i >= 0 && !trobat)
	{
		trobat = true;

		for (int f = nCasMax - 1; f >= 0; f--)
		{
			for (int c = 0; c < nCasMax; c++)
			{
				if (matriuAux[f][c])
				{
					if (m_tauler.getColorTauler(i + f - nCasMax + 1, col + c) != COLOR_NEGRE)
						trobat = false;
				}
			}
		}
	...
	if (trobat)
	{
		if (tipus == FIGURA_I)
		{
			switch (m_figuraActual.getFormaAct())
			{
			case 0:
			case 1: i -= 2;
				break;
		...
		m_figuraActual.setPosActFil(i);
		m_tauler.inserirFigura(m_figuraActual);
	}
}

bool Joc::apareixerFigura(int tipus, int fil, int col, int nGirs)
{
	m_figuraActual.setTipus(convertirEnTipusFigura(tipus));
	m_figuraActual.setPosActFil(fil);
	m_figuraActual.setPosActCol(col);
	m_figuraActual.setFormaAct(0);	//això ho modifiquem a l'hora de girar la figura
	m_figuraActual.setColor(convertirEnColorFigura(tipus));
	m_figuraActual.inicialitzarMatriuAuxiliar();
	for (int i = 0; i < nGirs; i++)
		m_figuraActual.girarFigura(GIR_HORARI); //girem la figura
	return m_tauler.inserirFigura(m_figuraActual);
}