...
bool Tauler::inserirFigura(const Figura& f)	//insereix una figura al tauler (si es pot)
{
	bool esPotInserir = true;
	int fila = 0, columna = 0;
	f.calcularPosicioTauler(fila, columna);
	
	if (fila < 0 || fila > N_FILES_TAULER - 1 || columna < 0 || columna > N_COL_TAULER - 1) //comprova si la figura està fora dels límits del tauler
		esPotInserir = false;
	...
	while (i < nCasMax && aux)	//fem una cerca en la matriu 
	{
		while (j < nCasMax && aux)
		{
			if (matriuAux[i][j])	//si hi ha peça
			{
				...
				{
					if (m_tauler[fila + i + 1][columna + j] != COLOR_NEGRE || (fila + i + 1) == N_FILES_TAULER)	//comprova si la posició sota la peça està buida o si ens trobem a la última fila del tauler
						aux = false;
				}
			}
			j++;
...
int Tauler::eliminaFiles(bool filesEliminadesArray[], int longitud, Figura& f) //elimina les files que estan plenes, return: número de files eliminades
{
	...
	for (int i = 0; i < longitud; i++) //recórrem les files de la matriu
	{
		...
		if (hiHaPeca) //si hi ha peça, comprovem si aquella fila del tauler està plena o no
		{
			...
			while (j < N_COL_TAULER && !casellaNegre) //cerca de la fila
			{
...
void Tauler::baixarFiles(bool filesEliminadesArray[], int longitud, int filaFigura, Figura& f) //baixa les files que estan per sobre de les files eliminades
{
	...
	for (int i = 0; i < longitud; i++) //recórrem l'array de les files eliminades
	{
		if (filesEliminadesArray[i]) //si la fila ha estat eliminada
		{
			...
			{
				for (int j = 0; j < N_COL_TAULER; j++)	//baixem totes les files que estaven per sobre de la fila eliminada, començant des d'abaix fins a dalt
					m_tauler[fila][j] = m_tauler[fila - 1][j];
			}

			for (int j = 0; j < N_COL_TAULER; j++) //posem la fila que queda més amunt en negre (eliminada), ja que s'ha baixat i s'ha quedat buida
	...
	case 1: //cap a la dreta
		if (f.getTipus() != FIGURA_I) //per la figura I ho fem diferent
		{
			while (i < nCasMax && aux) //fem una cerca en la matriu 
			{
				while (j < nCasMax && aux)
				{
					if (matriuAux[i][j]) //si hi ha peça
					{
						...
						{
							if (m_tauler[fila + i][columna + j + 1] != COLOR_NEGRE || (columna + j + 1) == N_COL_TAULER) //comprovar si la posició a la dreta de la peça està buida o si ens trobem a la última columna del tauler
								aux = false;
						}
					}
					j++;
			...
			if (f.getFormaAct() == 0 || f.getFormaAct() == 2) //la "I" està en horitzontal
			{
				...
				if (m_tauler[filaI][columna + 4] != COLOR_NEGRE || (columna + 4) == N_COL_TAULER) //estan totes les peces en la mateixa fila, només hem de comprovar que a la dreta de la última peça estigui buit o si ens trobem a la última columna del tauler
					aux = false;
			}
			else //la "I" està en vertical
			{
				...
				{
					if (m_tauler[fila + i][columnaI + 1] != COLOR_NEGRE || (columnaI + 1) == N_COL_TAULER) //comprovar si la posició a la dreta de la peça està buida o si ens trobem a la última columna del tauler
						aux = false;
					i++;
				}
			}
...
{													//ja que quan comprovo si es pot girar o no, elimino la figura, llavors aprofito y la torno a inserir aquí, així tenim només una funció
	...
	if ((forma == 0 && fila == (N_FILES_TAULER - 1)) || (tipus == FIGURA_I && forma == 0 && (fila == 0 || fila >= N_FILES_TAULER - 2)))
		esPotGirar = false;
	else
		if ((forma == 1 && columna == 0) || (tipus == FIGURA_I && forma == 1 && (columna < 2 || columna >= N_COL_TAULER - 1)))
			esPotGirar = false;
		else
			if ((forma == 2 && fila == 0) || (tipus == FIGURA_I && forma == 2 && (fila < 2 || fila >= N_FILES_TAULER - 1)))
				esPotGirar = false;
			else
				if ((forma == 3 && columna == (N_COL_TAULER - 1)) || (tipus == FIGURA_I && forma == 3 && (columna == 0 || columna >= N_COL_TAULER - 2)))
					esPotGirar = false;
				else
	...
	for (int i = 0; i < N_FILES_TAULER; i++)
	{
		for (int j = 0; j < N_COL_TAULER; j++)
		{
			if (m_tauler[i][j] != COLOR_NEGRE)
			{
				...
				switch (color)
				{
				case COLOR_GROC: imatge = GRAFIC_QUADRAT_GROC;
					break;
				case COLOR_BLAUCEL: imatge = GRAFIC_QUADRAT_BLAUCEL;
					break;
				case COLOR_MAGENTA: imatge = GRAFIC_QUADRAT_MAGENTA;
					break;
				case COLOR_TARONJA: imatge = GRAFIC_QUADRAT_TARONJA;
					break;
				case COLOR_BLAUFOSC: imatge = GRAFIC_QUADRAT_BLAUFOSC;
					break;
				case COLOR_VERMELL: imatge = GRAFIC_QUADRAT_VERMELL;
					break;
				case COLOR_VERD: imatge = GRAFIC_QUADRAT_VERD;
					break;
				}
