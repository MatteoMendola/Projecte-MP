...
public:
	...
	void inicialitza(int mode, const string& nomFitxer); //inicialitza l'estat d'una partida a partir d'un fitxer
	...
	int baixaFigura(bool& esPot); //retorna un 'int' amb el nombre de files eliminades
	bool giraFigura(DireccioGir direccio); //retorna un booleà depenent si la figura s'ha pogut girar o no
	void dibuixa() const;
	void baixarFinal();
	bool apareixerFigura(int tipus, int fil, int col, int nGirs);
private:
	//ATRIBUTS:
	Tauler m_tauler;
